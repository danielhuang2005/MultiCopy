...
/*!
   ...
   \remarks В пределах одного приложения нельзя создавать несколько экземпляров
     объектов данного класса. Каждый последующий созданный объект увеличит
     счётчики.

   \remarks Не следует разрушать объект до завершения работы программы,
     поскольку при его разрушении другие экземпляры программы не будут знать о
...
class TCoreAppInstances : public QObject
{
    ...
    private :
        ...
        QLocalServer*  m_pLocalServer;   //!< "Сервер" для приёма сообщений.
